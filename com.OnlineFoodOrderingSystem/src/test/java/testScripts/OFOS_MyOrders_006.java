...
public class OFOS_MyOrders_006 extends OFOS_Login_002 {

	@Test()
	public synchronized void testCase_006() throws InterruptedException {
		...
		for (int i = 0; i < itemCount; i++) {
			...
			if (i >= 0 && i < restaurant.getNameofAllRestaurants().size()) {
				...
				test.log(LogStatus.INFO,
						"Clicking on the View menu button of " + restaurant.getNameofAllRestaurants().get(i).getText());
				...
				test.log(LogStatus.INFO, "Clicking on name of restaurant \"" + restaurant.getNameofAllRestaurants().get(i).getText()+"\"");
				...
				test.log(LogStatus.INFO,
						"Clicking on the image of restaurant \"" + restaurant.getNameofAllRestaurants().get(i).getText()+"\"");
			...
			if (dish.getAllItemsName().size() > 0) {
				...
				clickAction(home.getHomeLink());
				test.log(LogStatus.INFO,
						"Clicked on the Order Now button of " + home.getNameOfAllDishes().get(i).getText());
				clickAction(home.getOrderNowButtonOfAllDishes().get(i));
			...
			try {
				for (int j = 0; j < myOrdersPage.getDeleteButtonofAllItems().size(); j++) {
					// Click on the remove button of All items present in the My orders list
					test.log(LogStatus.INFO,
							"Clicking on the Delete button of " + myOrdersPage.getNameOfAllItems().get(j).getText());
					clickAction(myOrdersPage.getDeleteButtonofAllItems().get(j));
					try {
						driver.switchTo().alert().accept();
						// Screeen shots
						test.log(LogStatus.PASS, test.addScreenCapture(getPhoto(driver)));
					} catch (NoAlertPresentException e) {
						test.log(LogStatus.FAIL, "No alert present");
					}

				}
			} catch (Exception e) {
				test.log(LogStatus.FAIL, "Can not click on delete button");
			}
			try {
